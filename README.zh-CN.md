...
macOS 用户需先在 Codex 中配置提供商、认证方式和兼容模型，并在 Desktop 中确认请求成功。然后启用 **设置 → 高级 → 启用提供商切换**。此设置只显示提供商，不会登录或更改当前提供商。选择后，应用只更新 `model_provider` 并重新打开 Desktop；切回已保存账号时会恢复该账号凭证及 `openai`。模型和模型目录不会自动恢复，必要时需选择或配置兼容模型。现有对话不会迁移。
