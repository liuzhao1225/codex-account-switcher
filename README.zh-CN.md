...
4. **macOS 可选提供商切换：** 先在 Codex 中配置并测试提供商，再启用 **设置 → 高级 → 启用提供商切换**，即可在已保存账号与已配置提供商之间切换。
...
- 提供商发现和选择通过已安装的 Codex app-server 完成。`config/read` 返回的完整配置可能让内联凭证进入进程内存；切换器只为界面保留提供商 ID 和名称，不显示、记录或持久化配置读取返回的自定义提供商凭证。切换器不会独立读取提供商环境变量的值或调用提供商认证命令，只修改 `model_provider`。
...
macOS 和 Windows 用户需先在 Codex 中配置提供商、认证方式和兼容模型，并在 Desktop 中确认请求成功。然后启用 **设置 → 高级 → 启用提供商切换**。此设置只显示提供商，不会登录或更改当前提供商。选择后，应用只更新 `model_provider` 并重新打开 Desktop；切回已保存账号时会恢复该账号凭证及 `openai`。模型和模型目录不会自动恢复，必要时需选择或配置兼容模型。现有对话不会迁移。

自定义提供商凭证继续由 Codex 或提供商认证机制管理。原生 OpenAI API 登录在切换时单独保存与恢复；切换器不提供 API key 输入框。使用不同模型 ID 的完整 Desktop 往返测试仍待完成，详见 [提供商兼容性](docs/provider-compatibility.md)。
